...
import axios from "axios"
  ...
  export default function SearchStore() {
    ...
    const {shopName,shopAddress,owner} = location.state
   ...
   async function productData(){
       ...
       try {
           const products = await axios.post('http://localhost:3000/api/store-data/obtain-products',{owner,pageNo})
   ...
   if (navItem) {
    ...
    return (
      <div className="bg-white">
        <div className="border-2 border-black/[0.1] h-[50px] mt-5 w-[20%] mx-10 rounded text-center inline-block border-box pt-2 font-bold text-xl">{shopName}</div>
        <div className="border-2 border-black/[0.1] h-[50px] w-[40%] rounded mx-2 text-center inline-block border-box pt-2 font-bold text-xl">{shopAddress}</div>  
